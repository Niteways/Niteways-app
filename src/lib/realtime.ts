 ...
 *     // later: unsub();
 *
 * Mirrors `mobile-app/src/services/realtime.ts` so both halves of the venue
 * portal use the same wire-up.
 */
export function subscribeToTable<T extends Record<string, unknown> = Record<string, unknown>>(
  ...
  const ch = supabase
    .channel(channelName)
    // The Supabase realtime typings split the second argument by event literal,
    // which makes a dynamic `event` argument too narrow to satisfy. We've
    // validated the shape ourselves above.
